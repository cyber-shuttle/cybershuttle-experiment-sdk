...
sftp_connector = SFTPConnector(
    host="cyberwater.scigap.org",
    port=9000,
    username="isuru_janith",
    password=token.accessToken,
)
path_suffix = sftp_connector.upload_files(
    "/Users/isururanawaka/Documents/Cyberwater/poc2/resources/storage",
    "Default_Project",
...
experiment = data_model_client.configure_computation_resource_scheduling(
...
print(ex_id)
# launch experiment
api_server_client.launch_experiment(token, ex_id, gateway_id)

status = api_server_client.get_experiment_status(token, ex_id)

if status is not None:
    print("Initial state " + str(getattr(status, "state")))
while getattr(status, "state") <= 6:
    status = api_server_client.get_experiment_status(token, ex_id)
    time.sleep(30)
    print("State " + str(getattr(status, "state")))
